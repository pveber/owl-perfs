...
let mat_mat_mul n =
  ...
  let c' = owl_jk69 n in
  let lacaml =
    Bench.Test.create ~name:"Lacaml_mat_mat_mul" (fun () -> Lacaml.D.gemm c c)
  and owl =
    Bench.Test.create ~name:"Owl_mat_mat_mul" (fun () -> Owl.Mat.dot c' c') in
...
let mat_vec_mul n =
  ...
  let lacaml = Bench.Test.create ~name:"Lacaml_mat_vec_mul" (fun () -> Lacaml.D.gemv c v) in
  let owl = Bench.Test.create ~name:"Owl_mat_vec_mul" (fun () -> Owl.Mat.dot c' v') in
  Bench.make_command [lacaml ; owl]
